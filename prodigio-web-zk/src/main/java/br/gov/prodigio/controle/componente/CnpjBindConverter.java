...
public class CnpjBindConverter implements TypeConverter {
	@Override
	...
	public Object coerceToUi(Object val, Component comp) {
		...
		if (val != null && !"".equals(val) && comp != null) {
			if (val instanceof String) {
				...
				aux = StringHelper.preencheStringComCaracter(aux, '0', 14);
				return StringHelper.format("##.###.###/####-##", aux);
			}
	...
	private Class getClass(Component comp) {
		Class classe = null;
		try {
			if (comp instanceof CnpjboxBind) {
				CnpjboxBind cnpjboxBind = (CnpjboxBind) comp;
				String nomeDoCampo = "";
				if (!StringUtils.isEmpty(cnpjboxBind.getNomeDoObjeto())) {
					String[] nodos = cnpjboxBind.getNomeDoObjeto().split("\\.");
					nomeDoCampo = nodos[nodos.length - 1];
				} else {
					nomeDoCampo = cnpjboxBind.getId();
				}
				nomeDoCampo = nomeDoCampo.replace("Arg", "");
				classe = Reflexao.recuperaTipoDeRetornoDoMetodoGet(nomeDoCampo, cnpjboxBind.getObjectPai());
			}
			return classe;

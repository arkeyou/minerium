...
public class ListboxDet extends Listbox implements AfterCompose, FieldValidator {
	...
	public void afterCompose() {
		...
		if (parent instanceof DetBox) {
			DetBox detBox = (DetBox) parent;
			this.setNomeDoObjeto(detBox.getNomeDoObjeto());
		}
		if (!getCrud().equals("false")) {
			ctr.getProAnnotateDataBinderHelper().adicionaAnotacaoParaListaBoxDet(this);
		}
	}
	...
	public void onChildAdded(Component child) {
		Window window = ProCtr.findWindow(this);
		// 02-02-2016 CORREÇÃO DO ERRO: AO EXCLUIR UMA LINHA DE UM POPUP O OBJETO WINDOW RETORNA NULL
		if (window == null)
			return;
		...
		ctr.getProAnnotateDataBinderHelper().adicionaAnotacaoParaItensDaListaBoxDet(child);
